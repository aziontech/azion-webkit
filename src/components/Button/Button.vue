  ...
  <template v-if="href || type === 'link' || type === 'linkExternal' || type === 'linkSecondary'">
    <a :href="href" class="flex gap-3 w-fit cursor-pointer group">
      ...
      <span v-if="type === 'linkExternal' || type === 'linkSecondary' || type === 'link'">
        <svg width="10" height="10" :class="['group-hover:translate-x-[.1rem] -translate-x-[.1rem]  transition-transform relative top-1/2 -translate-y-1/2', type === 'linkExternal' ? 'rotate-0 group-hover:translate-y-0 translate-y-[.1rem]' : 'rotate-45']" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
