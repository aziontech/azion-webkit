<template>
  <template v-if="href || type === 'link' || type === 'linkExternal' || type === 'linkSecondary'">
    <a :href="href" class="flex gap-3 w-fit cursor-pointer group">
      <Button
        :label="label"
        :size="size"
        :icon="icon"
      ...
      <span v-if="type === 'linkExternal' || type === 'linkSecondary' || type === 'link'">
        <svg width="10" height="10" :class="['group-hover:translate-x-1 -translate-x-1  transition-transform relative top-[.1rem]', type === 'linkExternal' ? 'rotate-0 group-hover:translate-y-0 translate-y-[.1rem]' : 'rotate-45']" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script setup>
  ...
  const labelClasses = computed(() => {
    ...
    const underlineBase = 'relative after:content-[\'\'] after:absolute after:-bottom-[.1rem] after:left-0 after:w-0 after:h-[1px] after:transition-all after:duration-150 group-hover:after:w-full'

    return {
      primary: `font-proto-mono ${leading}`,
      secondary: `font-proto-mono ${leading}`,
      tertiary: `font-proto-mono ${leading}`,
      link: `font-proto-mono ${leading} ${fontSize} ${underlineBase} after:bg-violet-600`,
      linkSecondary: `font-proto-mono ${leading} ${fontSize} ${underlineBase} after:bg-neutral-900`,
      linkExternal: `font-proto-mono ${leading} ${fontSize} ${underlineBase} after:bg-violet-300`,
    }[props.type]
  })
</script>
