...
const MOCK = {
  ...
  "title": "We are committed to ensuring that our global customers and partners can meet diverse compliance requirements.",
  "description": "Security and Compliance are shared responsibilities between Azion and the customer. This shared model can help relieve the customer’s operational burden as Azion operates, manages and controls the components from the operating system and virtualization layer, including updates and security patches, down to the physical security of the facilities in which the service operates.",
  "buttons": [
    {
  ...
  <ContentSection position="right" :overline="args.overline" :title="args.title" :description="args.description">
    ...
    <template #main>
      <div class="w-full max-w-80">
        <div class="hidden lg:block lg:top-20 lg:sticky">
          <Menu :model="args.leftSidebar" class="w-full" :pt="{
    ...
    <template #content>
      <div class="flex flex-col gap-10 lg:gap-20 mt-12">
        <div v-for="({ overline, id, cards }) in args.cardContent" :id="id" class="flex flex-col gap-6">
          <Overline :label="overline" />
          <CardBase v-for="({ title, text, links, image, alt }) in cards" class="w-full" spacing="compact">
            <template #content>
              <div class="flex flex-col lg:flex-row gap-8 lg:gap-3">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 lg:gap-5 m-0 lg:w-11/12">
                  ...
                  <CardDescription class="xl:col-span-2 lg:row-span-2"> {{ text }} </CardDescription>
                </div>
              </div>
            </template>
          </CardBase>
