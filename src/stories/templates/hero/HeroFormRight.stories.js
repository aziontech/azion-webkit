...
const logos = Array(3).fill({
  imageSrc: `<svg xmlns="http://www.w3.org/2000/svg" width="75" height="18" fill="currentColor" viewBox="0 0 75 18">
  ...
  <HeroBlockBase align="center" :title="args.hero.title">
    <template #actions>
      <div class="flex lg:justify-start justify-center gap-2 w-full">
        <LinkButton v-for="button in args.CTA" class="md:w-fit w-full" :outlined="button.outlined"
          :label="button.label" :link="button.href" />
      </div>
    </template>

    <template #content>
      <div class="w-full flex items-center justify-center lg:items-start lg:justify-start">
        <ContentLogoBlock title="+100 empresas são parceiras da Azion" :logos="args.logos" />
      </div>
    </template>

    <template #main>
      <FormBePartner class="w-full" requester="azion.com" requesterName="Seja um parceiro" :hubspot="args.hubspot"
