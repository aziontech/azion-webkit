<template>
  <div class="flex flex-col justify-center items-center w-full">
    ...
    >
      ...
      >
        ...
        <button
          @click="toggleTheme"
          class="z-20 hidden md:flex items-center justify-center h-12 w-12"
        >
          <i :class="getThemeIcon()"></i>
        </button>
        <button
          @click="toggleBottombar"
          class="z-20 flex items-center justify-center h-12 w-12 md:hidden"
        >
    ...
    <div class="flex flex-col pb-8 w-full max-w-[1536px]">
      ...
      >
        ...
        <!-- Comandos -->
        <div
          class="flex-col gap-3 w-full md:flex-col md:w-64 md:sticky top-28 md:h-60 z-10 hidden md:flex"
        >
          <div
            ...
            <div class="radio-group">
              ...
              <label class="w-full">
                <input
                  ...
                  v-model="downloadFormat"
                />
                <div class="radio-container">.svg</div>
              </label>
            </div>
          </div>
        </div>
  ...
  <div
    v-if="openBottombar"
    class="flex w-full h-fit bottom-0 fixed bg-white z-30 rounded-t-xl md:hidden bg-neutral-300/90 dark:bg-neutral-800/90 border border-neutral-200 dark:border-neutral-600"
  >
    <div
      class="flex flex-col gap-3 w-full md:flex-col md:w-64 md:sticky top-28 md:h-60 z-10 px-4 pt-6 pb-8"
    >
      <!-- Comandos -->
      <div
        class="flex items-center gap-4 bg-neutral-100/60 py-2 border border-neutral-200 rounded-lg px-2.5 dark:bg-neutral-800/60 dark:border-neutral-700"
      >
        ...
        <div class="flex flex-row w-full items-center gap-2">
          <input
            ...
            class="w-full slider-input"
          />
          <p class="text-neutral-500 dark:text-neutral-400 w-10 text-xs">
            {{ selectedFontSize }}px
          </p>
        </div>
      </div>
      ...
      <button
        @click="toggleTheme"
        class="flex items-center justify-center h-12 w-full"
      >
        <i :class="getThemeIcon()"></i>
      </button>
    </div>
